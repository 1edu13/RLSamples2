...
dqn_module = importlib.import_module("11_pendulum_dqn")
...
def evaluate_model(env_name, agent_type, episodes=10):
    ...
    action_dim = 3 if is_discrete else env.action_space.shape[0]
        
    # Inicializar y Cargar pesos del Agente correspondiente
    agent_instance = None
    if agent_type == "DQN":
        agent_instance = dqn_module.DQNAgent(state_shape, action_dim)
        path = get_latest_model_path("models", "DQN_Pendulum", "best_model.pth")
        if path:
        ...
        path = get_latest_model_path("models", "TD3_Pendulum", "best_actor.pth")
        if path:
        ...
        path = get_latest_model_path("models", "PPO_Pendulum", "best_policy.pth")
        if path:
        ...
        path = get_latest_model_path("models", "SAC_Pendulum", "best_actor.pth")
        if path:
            agent_instance.actor.load_state_dict(torch.load(path, map_location=agent_instance.device))
        else:
    ...
    for ep in range(episodes):
        ...
        # En Pendulum fallar es no poder levantarlo (recompensa acumulada muy negativa)
        exito = (ep_reward > -250)
        if not exito:
            crashes += 1
            
        print(f"  > Ep {ep+1}/{episodes} | Recompensa: {ep_reward:.2f} | ¿Falló?: {'Sí' if not exito else 'No'}")
...
if __name__ == "__main__":
    ...
    for algo in algos:
        avg_r, cr = evaluate_model("Pendulum-v1", algo, episodes=10)
    ...
    try:
        ...
        ax2 = ax1.twinx()
        color = 'tab:red'
        ax2.set_ylabel('Tasa de Fallos (%)', color=color)
        rects2 = ax2.bar(x + width/2, crashes, width, label='Fallos (%)', color=color, alpha=0.7)
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.set_ylim([0, 100])

        ax1.set_xticks(x)
        ax1.set_xticklabels(labels)
        plt.title('Comparación de Modelos Entrenados en Pendulum-v1')
